            ...
            <div class="card overflow-hidden mb-8">
              <div class="grid md:grid-cols-2 gap-0">
                ...
                <div class="relative h-96 md:h-auto overflow-hidden bg-slate-200">
                  <div
                    class="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent z-10"></div>
                  <img
                    [src]="drillData.imageUrl || 'https://images.unsplash.com/photo-1566577739112-5180d4bf9390?w=800&h=600&fit=crop'"
                    [alt]="drillData.name"
                    class="w-full h-full object-cover"
                  />
                  <div class="absolute top-4 right-4 z-20 flex gap-2">
                    <span [class]="getLevelBadgeClass(drillData) + ' shadow-lg backdrop-blur-sm'">
                ...
                <div class="p-8 flex flex-col">
                  <h1 class="text-4xl font-display font-bold text-gray-900 mb-4">{{ drillData.name }}</h1>

                  <div class="grid grid-cols-2 gap-4 mb-6">
                    <div class="bg-green-50 rounded-xl p-4 border border-green-100">
                      <div class="text-sm text-green-600 font-medium mb-1">{{ 'DRILL_DETAIL.DURATION' | translate }}
                      </div>
                    ...
                    <div class="bg-slate-50 rounded-xl p-4 border border-slate-100">
                      <div class="text-sm text-slate-600 font-medium mb-1">{{ 'DRILL_DETAIL.CATEGORY' | translate }}
                      </div>
                      <div class="text-lg font-bold text-slate-700">
                        <span [class]="getCategoryBadgeClass(drillData)">
            ...
            <div class="grid md:grid-cols-3 gap-8">
              ...
              <div class="md:col-span-2 space-y-6">
                ...
                <div class="card p-6">
                  ...
                  <p class="text-gray-700 leading-relaxed">{{ drillData.description }}</p>
                </div>

                <!-- Instructions -->
                @if (drillData.instructions && drillData.instructions.length > 0) {
                  <div class="card p-6">
                    <h2 class="text-2xl font-display font-bold text-gray-900 mb-4 flex items-center">
                      <svg class="w-6 h-6 mr-2 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <ol class="list-decimal list-inside space-y-2 text-gray-700">
                      @for (instruction of drillData.instructions; track $index) {
                        <li>{{ instruction }}</li>
                      }
                    </ol>
                  </div>
              ...
              <div class="space-y-6">
                <!-- Equipment -->
                @if (drillData.equipment && drillData.equipment.length > 0) {
                  <div class="card p-6">
                ...
                <!-- Coaching Tips -->
                @if (drillData.coachingTips && drillData.coachingTips.length > 0) {
...
export class DrillDetailComponent implements OnInit {
  ...
  getCategoryBadgeClass(drillData: Drill): string {
    const baseClass = 'badge border text-sm';
    switch (drillData.category) {
      case 'passing':
