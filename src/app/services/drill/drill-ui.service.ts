...
export class DrillUiService {
  ...
  getLevelTranslationKey(value: string): string {
    ...
    const match = DRILL_LEVELS.find(l => l.value.toLowerCase() === value.toLowerCase());
    return match ? match.translationKey : value;
  }
  ...
  getCategoryTranslationKey(value: string): string {
    ...
    const match = DRILL_CATEGORIES.find(c => c.value.toLowerCase() === value.toLowerCase());
    return match ? match.translationKey : value;
  }

  getEquipmentTranslationKey(equipmentValue: string): string {
    if (!equipmentValue) {
      return '';
    }
    return equipmentValue.toUpperCase();
  }
}
